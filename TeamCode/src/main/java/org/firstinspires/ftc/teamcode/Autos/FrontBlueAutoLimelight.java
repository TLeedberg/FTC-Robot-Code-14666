...
public class FrontBlueAutoLimelight extends LinearOpMode {
    ...
    public void shooterAuto(double rpm, long upTime, long fireTime){
        ...
        shooterMotorA.setPIDFCoefficients(DcMotor.RunMode.RUN_USING_ENCODER, pidfCoefficients);
        shooterMotorB.setPIDFCoefficients(DcMotor.RunMode.RUN_USING_ENCODER, pidfCoefficients);
        shooterMotorA.setVelocity((rpm/60)*28);
        shooterMotorB.setVelocity((rpm/60)*28);
    ...
    public void servoDown(){
        rightFinger.setPosition(0.11);
        leftFinger.setPosition(0.42);
    }
    ...
    public void autoAim() {
        double error = turretAimCamera();
        while (Math.abs(error) > 1){
            error = turretAimCamera();
        }
    }

    public double turretAimCamera() {
        LLResult llResult = limeLight.getLatestResult();
        if (llResult != null && llResult.isValid()){
            double turret_error = -(P * llResult.getTx());
            if ((Math.abs(llResult.getTx()) > 1) && llResult.isValid()){
                turretMotor.setPower(turret_error);
            }else{
                turretMotor.setPower(0);
            }
            double dk = 32722.097;
            double distance = Math.sqrt(dk/llResult.getTa());
            telemetry.addData("I can see april tag", llResult.getBotposeTagCount());
            telemetry.addData("Tx", llResult.getTx());
            telemetry.addData("Ty", llResult.getTy());
            telemetry.addData("Ta", llResult.getTa());
            telemetry.addData("Distance from april tag is ", distance);
            ...
            turretMotor.setPower(0);
            telemetry.addData("I can not see april tag :(", llResult.getBotposeTagCount());
            telemetry.update();
            return 0;
        }
    }
